...
HOSTNAME=$(grep 'Allow cross origin' /home/jboss/amq-broker/etc/jolokia-access.xml | grep 'Allow cross origin' | sed 's/.*\(cray.*local\).*/\1/' | sed 's/.*\(cray.*local\).*/\1/')
USERNAME=$(grep cluster-user /home/jboss/amq-broker/etc/broker.xml  | sed 's/.*>\(.*\)<.*/\1/')
PASSWORD=$(grep cluster-password /home/jboss/amq-broker/etc/broker.xml  | sed 's/.*>\(.*\)<.*/\1/')

if curl -u \"${USERNAME}:${PASSWORD}\" http://${HOSTNAME}:8161/console/jolokia/read/org.apache.activemq.artemis:broker=\"amq-broker\"/Backup | grep -q true; then
  echo $POD_NAME is currently a backup. > ${OUTPUT}
  exit 0
fi
